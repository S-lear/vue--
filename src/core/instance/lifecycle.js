...
export function initLifecycle (vm: Component) {
  // vm.$options = {el: '#app', data: {a: 1, b: [1, 2, 3]}, components, directives, filters, _base}，
  const options = vm.$options
  // console.log('options.....', options)
  // locate first non-abstract parent // 查找到第一个非抽象组件
  let parent = options.parent // options.parent = undefined
  if (parent && !options.abstract) { // 查看vue官方文档api(keep-alive)即可知道什么是抽象和非抽象的区别
    while (parent.$options.abstract && parent.$parent) { // 直到找到非抽象组件
      parent = parent.$parent
    }
    parent.$children.push(vm) // 存在父实例则把当前vm存入$children
  }

  vm.$parent = parent // 指定已创建的实例之父实例，在两者之间建立父子关系。子实例可以用 this.parent 访问父实例，子实例被推入父实例的children 数组中。
