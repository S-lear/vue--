...
function mergeOptions (
  parent, // parent = {components, directives, filters}
  child, // child = {el: '#app', data: {a: 1, b: [1, 2, 3]}}
  vm // this
) {
  if (process.env.NODE_ENV !== 'production') {
...
function initLifecycle (vm) {
  // vm.$options = {el: '#app', data: {a: 1, b: [1, 2, 3]}, components, directives, filters}，
  var options = vm.$options;
...
function initMixin (Vue) {
  Vue.prototype._init = function (options) {
    ...
    vm._isVue = true;
    // merge options
    if (options && options._isComponent) { // 内部使用_isComponent,不予理会
      // optimize internal component instantiation
      // since dynamic options merging is pretty slow, and none of the
      // internal component options needs special treatment.
      initInternalComponent(vm, options);
      ...
      // 此处 vm.$options = {el: '#app', data: {a: 1, b: [1, 2, 3]}, components, directives, filters}，
...
function Vue (options) {
  if (process.env.NODE_ENV !== 'production' && // NODE_ENV 开发时使用
    !(this instanceof Vue) // 判断this是否指向new Vue()
  ) {
